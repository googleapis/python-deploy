...
class CloudDeployClient(metaclass=CloudDeployClientMeta):
    ...
    @staticmethod
    def job_run_path(project: str,location: str,delivery_pipeline: str,release: str,rollout: str,job_run: str,) -> str:
        """Returns a fully-qualified job_run string."""
        return "projects/{project}/locations/{location}/deliveryPipelines/{delivery_pipeline}/releases/{release}/rollouts/{rollout}/jobRuns/{job_run}".format(project=project, location=location, delivery_pipeline=delivery_pipeline, release=release, rollout=rollout, job_run=job_run, )

    @staticmethod
    def parse_job_run_path(path: str) -> Dict[str,str]:
        """Parses a job_run path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/deliveryPipelines/(?P<delivery_pipeline>.+?)/releases/(?P<release>.+?)/rollouts/(?P<rollout>.+?)/jobRuns/(?P<job_run>.+?)$", path)
        return m.groupdict() if m else {}
